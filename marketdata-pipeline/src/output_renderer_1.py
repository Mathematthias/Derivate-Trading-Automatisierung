...
def _render_setup_class_flags(snapshots: dict[str, TickerSnapshot]) -> list[str]:
    ...
    # Indizes/Forex/Krypto/Futures sind ausgeschlossen. Die Exclusion greift
    # seit V1.1-Fix (2026-05-27) bereits auf Computation-Ebene
    # (market_data._compute_anomaly_fields) — has_any_anomaly ist für solche
    # Symbole ohnehin False. Der Filter hier bleibt als Defense-in-Depth.
    from market_data import is_anomaly_excluded_symbol

    anomaly_snaps: list[TickerSnapshot] = [
        s for s in snapshots.values()
        if s.has_any_anomaly and not is_anomaly_excluded_symbol(s.symbol)
    ]
