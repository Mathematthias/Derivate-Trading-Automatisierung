# ROUTINE: Derivate Morning-Check (V5, Markdown-File-Pattern)
# Trigger: 08:45 Mo–Fr, Timezone Europe/Berlin (Cron: 45 8 * * 1-5)
# Ausführung: autonom, kein User-Dialog möglich
# Version: 1.5 (.md-File statt Doc-Konvertierung, Content-Limit 1500)
...
**Dateiname:** `Briefing-YYYY-MM-DD.md` — **aktuelles Europe/Berlin-Datum** (nicht UTC).
...
- Maximale Länge des Briefing-Texts: **1500 Zeichen UTF-8**. Nicht mehr.
- Empirisch: V1.4 mit 3000er-Limit hat bei ~1800 Zeichen im Drive-API-Call den
  Stream-Idle-Timeout gerissen. 1500 ist der sichere Puffer.
- Falls Abschnitte länger würden: Watchlist nur Kandidaten mit Trigger <10% entfernt,
  Top-Kandidaten max 2, Quellen-Footer max 3 Zeilen, keine Bullet-Duplikate.
- Vor dem Tool-Call Zeichen zählen, notfalls kürzen — lieber prägnant als abgeschnitten.
...
**Warum Markdown-File statt Google Doc:** V1.2–V1.4 nutzten
`text/plain → application/vnd.google-apps.document` als Konvertierung. Die
Drive-interne Doc-Engine hat bei jedem Run den Stream-Idle-Timeout gerissen,
selbst bei mäßig großem Content. `.md`-File ohne Konvertierung ist nur Storage,
keine Doc-Konstruktion — schnell und timeout-sicher. Drive zeigt `.md`-Files
inline als Text an, und der Chat-Reader liest sie ohnehin per
`read_file_content`.
...
- **Kein** `mimeType="text/plain"` und **kein** Weglassen von
  `disableConversionToGoogleType=true`. Beides würde zurück in die
  Doc-Konvertierungs-Falle führen.
- Base64 wird **direkt** im Tool-Call-Argument als String übergeben. Modell
  encodet selbst.
