...
# Ausführung: autonom
# Version: 1.4 (STATE-Doc-ID hart, create_file direkt, Content-Limit 2k)
...
**Dateiname:** `Scan-YYYY-MM-DD-1545` — **Europe/Berlin-Datum**
...
- Maximale Länge Scan-Text: **2000 Zeichen UTF-8**.
- Watchlist nur Treffer <10% entfernt, Quellen-Footer max 4 Zeilen.

**Tool-Call — genau so, ohne Umwege:**

```
Google Drive:create_file(
  title      = "Scan-YYYY-MM-DD-1545",
...
Drive konvertiert `text/plain` automatisch zu Google Doc.

**⛔ VERBOT:**
- **Kein** Python-Subprocess (`cat > /tmp/*.py`, `python3 /tmp/*.py`) zum Encoden
- **Kein** Zwischenschritt-Doc in /tmp
- **Kein** `disableConversionToGoogleType=true`
- Base64 direkt im Tool-Call-Argument, Modell encodet selbst

**Falls Name-Kollision** (Re-Run): Suffix `-v2`, `-v3`. Erkennung per `search_files`
mit `title = 'Scan-YYYY-MM-DD-1545' and parentId = '1jKuyo12c38sg8Ff4ZHFXHqsU_Nl2D4AA'`.
